...
class M_Anggota_Jemaat extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT count(id_anggota) as jumlahJemaat FROM anggota_jemaat WHERE status_anggota = 'AKTIF'");
        return $query;
    }
    ...
    {
        return $this->db->query("SELECT count(jenis_kelamin_anggota) as jumlahLakilaki FROM anggota_jemaat WHERE jenis_kelamin_anggota = 'Laki-laki' AND status_anggota = 'AKTIF' ");
    }

    public function tampil_jemaat_perempuan()
    {
        return $this->db->query("SELECT count(jenis_kelamin_anggota) as jumlahPerempuan FROM anggota_jemaat WHERE jenis_kelamin_anggota = 'Perempuan' AND status_anggota = 'AKTIF'");
    }
    ...
    public function tampil_jemaat_ulang_tahun($awal)
    {
        return $this->db->query("SELECT nama_lengkap_anggota, nama_wilayah, tanggal_lahir_anggota FROM anggota_jemaat JOIN wilayah ON anggota_jemaat.id_wilayah = wilayah.id_wilayah  
                                WHERE DAY('$awal') = DAY(tanggal_lahir_anggota) AND MONTH('$awal') = MONTH(tanggal_lahir_anggota) AND status_anggota = 'AKTIF'");
    }
